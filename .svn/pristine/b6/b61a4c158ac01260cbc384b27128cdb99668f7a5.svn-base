jQuery(document).ready(function () {
    ...
    jQuery('.slider-centered .slides li').each(function () {
    ...
    jQuery(window).scroll(function () {
        ...
        if (iOS === false) {
            ...
            jQuery('.has-parallax').each(function () {
                ...
                var scrollAmount = (jQuery(window).scrollTop() - jQuery(this).offset().top) / 3;
                scrollAmount = Math.round(scrollAmount);
                jQuery(this).css('background-position', '50% ' + scrollAmount + 'px');
...
jQuery(document).ready(function($){
    ...
    $('section').waypoint(function()
    {
        ...
        $('[data-target=' + $(this).attr('data-nav') + ']').addClass('active');
    }, {
        offset: '90px'
    });
    ...
    $(window).scroll(function(){
        if(!$('#secondary-navigation').hasClass('visible'))
        {
            $('#secondary-navigation').addClass('visible');
        }
    });
    ...
    $(window).scrollStopped(function(){
        if($('#secondary-navigation').hasClass('visible'))
        {
            $('#secondary-navigation').removeClass('visible');
        }
    });

    $('.up-nav').on('click touchstart', function(){
        if($('#secondary-navigation .active').attr('data-target') !== 'home')
        {
            $('#secondary-navigation .active').prev().find('a').trigger('click');
        }
    });

    $('.down-nav').on('click touchstart', function(){
        if($('#secondary-navigation .active').attr('data-target') !== 'contact')
        {
            $('#secondary-navigation .active').next().find('a').trigger('click');
        }
    });

    setTimeout(function(){
        $('.selector-image').height($('.selector-image').width());
    },0);
});